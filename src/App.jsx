...
export default function App() {
  ...
  const handleAddResult = async () => {
    ...
    if (parsed.length === 0) { setSaveStatus("⚠️ ไม่พบขอมูลที่ถกรูปแบบ"); return; }
  ...
  const handleAddClosed = async () => {
    ...
    const existing = allData[inputDate.trim()];
    if (!existing) { setSaveStatus("⚠️ ยงไม่มีผลหวยวนนี้"); return; }
  ...
  const AdminPrompt = () => (
    <>
      ...
      <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 301, ...glassStrong, padding: "32px 28px", width: "85%", maxWidth: 320, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>Admin เท่านน</div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginBottom: 24 }}>ใสรหัสเพอดำเนนการต่อ</div>
        <input
          type="password"
          value={adminInput}
          autoFocus
        ...
        <div style={{ padding: "20px 16px", position: "relative", zIndex: 1 }}>
          {latest && (
            <div style={{ ...glassStrong, padding: "28px 20px", marginBottom: 20, textAlign: "center", background: `linear-gradient(135deg, ${accent}14, rgba(255,255,255,0.05))`, borderColor: `${accent}40` }}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>งวดวันที่</div>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <div style={{ flex: 1, borderRight: "1px solid rgba(255,255,255,0.08)" }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 6 }}>3 ตัวบน</div>
                  <div style={{ fontSize: 32, fontWeight: 800, color: accent }}>{latest.top3}</div>
                </div>
                <div style={{ flex: 1, borderRight: latest.closed?.length > 0 ? "1px solid rgba(255,255,255,0.08)" : "none" }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 6 }}>2 ตัวล่าง</div>
                  <div style={{ fontSize: 32, fontWeight: 800, color: "#c3fae8" }}>{latest.bot2}</div>
                </div>
                {latest.closed?.length > 0 && (
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1, marginBottom: 6 }}>เลขปิด</div>
          ...
          <div style={{ ...glass, overflow: "hidden", padding: 0 }}>
            ...
            {history.map((h, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 80px 70px 80px", padding: "13px 16px", borderTop: i > 0 ? "1px solid rgba(255,255,255,0.05)" : "none", background: i === 0 ? `${accent}0d` : "transparent", alignItems: "center" }}>
                ...
                <span style={{ textAlign: "center", fontSize: 17, fontWeight: 700, color: "#c3fae8" }}>{h.bot2}</span>
    ...
    <div style={baseStyle}>
        ...
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 201, ...glassStrong, borderRadius: "28px 28px 0 0", padding: "12px 20px 44px" }}>
            ...
            <button onClick={() => { setTab("view"); setMenuOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", borderRadius: 18, border: "none", cursor: "pointer", background: tab === "view" ? "rgba(116,143,252,0.2)" : "rgba(255,255,255,0.06)", color: tab === "view" ? "#748ffc" : "#fff", fontFamily: "inherit", fontSize: 16, fontWeight: tab === "view" ? 700 : 500, marginBottom: 8, textAlign: "left" }}>
              ...
              <span>ดูผลหวย</span>
              {tab === "view" && <span style={{ marginLeft: "auto", fontSize: 11, color: "#748ffc" }}>● กำลงใช้</span>}
            </button>
            ...
            {activeDate && (
              <>
                ...
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12, paddingLeft: 4 }}>จดการ · {activeDate}</div>
                {/* ลบ - ต้องใส่รหัส */}
      ...
      <div style={{ ...glassStrong, borderRadius: "0 0 32px 32px", padding: "54px 20px 18px", position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 2, textTransform: "uppercase", marginBottom: 3 }}>สรปผล</div>
          ...
          <>
            ...
            {!statsBreakdown ? (
              ...
              <div style={{ ...glassStrong, padding: "22px 20px", background: `linear-gradient(135deg, ${statsAccent}14, rgba(255,255,255,0.05))`, borderColor: `${statsAccent}40` }}>
                ...
                <StatRow label="เลขลงท้ายเลขค" value={statsBreakdown.evenOdd[0]} accent={statsAccent} />
          ...
          <>
            {dates.length === 0 ? (
              <div style={{ textAlign: "center", padding: "80px 20px", color: "rgba(255,255,255,0.25)" }}>
                ...
                {loaded && <div style={{ marginTop: 8, fontSize: 13 }}>กดปม ☰ มุมขวาบนเพื่อเพิ่มข้อมูล</div>}
              </div>
            ) : (
              <>
                ...
                {activeDate && (
                  <div style={{ ...glass, display: "flex", justifyContent: "space-around", alignItems: "center", padding: "16px 20px", marginBottom: 16 }}>
                    <div style={{ textAlign: "center" }}>
                      ...
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 1, textTransform: "uppercase" }}>รายการ</div>
                    </div>
                    <div style={{ width: 1, height: 32, background: "rgba(255,255,255,0.1)" }} />
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 24, fontWeight: 800, color: "#a9e34b" }}>{Object.keys(grouped).length}</div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 1, textTransform: "uppercase" }}>ประเทศ</div>
                    </div>
                    <div style={{ width: 1, height: 32, background: "rgba(255,255,255,0.1)" }} />
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 14, fontWeight: 700 }}>{activeDate}</div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 1, textTransform: "uppercase" }}>วันที่</div>
                    </div>
                  </div>
                )}
                ...
                {Object.entries(grouped).map(([country, rows]) => {
                  ...
                  return (
                    <div key={country} style={{ marginBottom: 16 }}>
                      ...
                      <div style={{ ...glass, overflow: "hidden", padding: 0 }}>
                        <div style={{ display: "grid", gridTemplateColumns: hasClosed ? "1fr 70px 60px 76px" : "1fr 80px 70px", padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, textTransform: "uppercase" }}>
                          <span>ชื่อหวย</span>
